...
namespace com.Sconit.Service.MasterData.Impl
{
    ...
    public class PartyMgr : PartyBaseMgr, IPartyMgr
    {
        ...
        public IList<Party> GetOrderFromParty(string orderType, string userCode, bool includeInactive)
        {
            ...
            if (orderType == BusinessConstants.CODE_MASTER_FLOW_TYPE_VALUE_PROCUREMENT || orderType == BusinessConstants.CODE_MASTER_FLOW_TYPE_VALUE_SUBCONCTRACTING)
            {
                DetachedCriteria[] pfCrieteria = SecurityHelper.GetPartyPermissionCriteria(userCode,new string[]{BusinessConstants.CODE_MASTER_PARTY_TYPE_VALUE_REGION,BusinessConstants.CODE_MASTER_PARTY_TYPE_VALUE_SUPPLIER});
                criteria.Add(
                    Expression.Or(
                        Subqueries.PropertyIn("Code", pfCrieteria[0]),
                        Subqueries.PropertyIn("Code", pfCrieteria[1])
        ...
        public IList<Party> GetToParty(string orderType, string userCode, bool includeInactive)
        {
            ...
            {
                DetachedCriteria[] pfCrieteria = SecurityHelper.GetPartyPermissionCriteria(userCode, new string[] { BusinessConstants.CODE_MASTER_PARTY_TYPE_VALUE_REGION, BusinessConstants.CODE_MASTER_PARTY_TYPE_VALUE_CUSTOMER });
                criteria.Add(
                    Expression.Or(
                        Subqueries.PropertyIn("Code", pfCrieteria[0]),
                        Subqueries.PropertyIn("Code", pfCrieteria[1])
