...
namespace com.Sconit.Service.Report.Impl
{
    ...
    public class RepBillMgr : RepTemplate1
    {
        ...
        public IKPOrderMgrE kpOrderMgrE { get; set; }
        ...
        protected override void FillValuesImpl(String templateFileName, IList<object> list)
        {
            ...
            foreach (KPItem kpItem in kpItems)
            {
                ...
                {
                    this.SetRowCell(pageIndex, rowIndex, 5, ((decimal)kpItem.PRICE).ToString("0.00"));
                }
                //单位	
                ...
                {
                    this.SetRowCell(pageIndex, rowIndex, 7, ((decimal)kpItem.PRICE1).ToString("0.00"));
                }
                //发票单价 @金额	
                if (kpItem.PRICE1 != null)
                {
                    this.SetRowCell(pageIndex, rowIndex, 8, ((decimal)kpItem.PRICE2).ToString("0.00"));
                ...
                {
                    //合计发票金额：
                    this.SetRowCell(pageIndex, this.pageDetailRowCount, 10, totalPrice.ToString("0.00"));
                    rowIndex = 0;
                    pageIndex++;
                    totalPrice = 0;
                }
        ...
        public override IList<object> GetDataList(string code)
        {
            ...
            KPOrder kpOrder = kpOrderMgrE.LoadKPOrder(decimal.Parse(code), true);
            if (kpOrder != null)
            {
                list.Add(kpOrder);
