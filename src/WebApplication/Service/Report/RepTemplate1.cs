...
namespace com.Sconit.Service.Report
{
    ...
    public abstract class RepTemplate1 : ReportBaseMgr
    {
         ...
         * Param row2 终止行号(相对值)
         * Param colunm2 终止列号
         * Param includeHeadRowCount 是否包含头行数
         */
        protected void SetMergedRegion(int pageIndex, int row1, int column1, int row2, int colunm2, bool includeHeadRowCount)
        {
            if (includeHeadRowCount)
            {
                SetMergedRegion(pageIndex, row1, column1, row2, colunm2);
            }
            else
            {
                SetMergedRegion(pageIndex, row1 - this.headRowCount, column1, row2 - this.headRowCount, colunm2);
            }
        }
        ...
        */
        ...
        protected abstract void FillValuesImpl(String templateFileName, IList<object> list);
    }
}
