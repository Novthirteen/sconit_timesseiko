    ...
    <div class="GridView">
        <cc1:GridView ID="GV_List" runat="server" AutoGenerateColumns="False" DataKeyNames="Id"
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="${MasterData.Item.Description}" SortExpression="Item.Description">
                    <ItemTemplate>
                        <asp:Label ID="GV_lbItemDescription" runat="server" Text='<%# DataBinder.Eval(Container.DataItem, "Item.Description")%>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="${MasterData.Item.Spec}" SortExpression="Item.Spec">
                    <ItemTemplate>
                        <asp:Label ID="GV_lbItemSpec" runat="server" Text='<%# DataBinder.Eval(Container.DataItem, "Item.Spec")%>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="${MasterData.Item.Brand}" SortExpression="Item.Brand">
                    <ItemTemplate>
                        <asp:Label ID="GV_lbItemBrand" runat="server" Text='<%# DataBinder.Eval(Container.DataItem, "Item.Brand")%>' />
                    </ItemTemplate>
                </asp:TemplateField>
