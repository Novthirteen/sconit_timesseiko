...
public partial class Login : System.Web.UI.Page
{
    ...
    protected void Login_Click(object sender, EventArgs e)
    {
        ...
        {
            if (user.EnableUsbKey)
            {
                if (this.UsbKey.Value == string.Empty)
                {
                    this.errorLabel.Text = "没有获取到UsbKey,请插入UsbKey身份认证!";
                    return;
                }
                else if (FormsAuthentication.HashPasswordForStoringInConfigFile(this.UsbKey.Value, "MD5") != user.UsbKey)
                {
                    this.errorLabel.Text = "UsbKey身份认证失败!";
                    return;
                }
            }
