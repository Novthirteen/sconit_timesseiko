    ...
    <EditItemTemplate>
        <table class="mtable">
            ...
            <tr>
                ...
                <td class="td01">
                    <asp:Literal ID="lbApprovalStatus" runat="server" Text="${InProcessLocation.ApprovalStatus}:" />
                    <%-- <asp:Literal ID="lblDockDescription" runat="server" Text="${InProcessLocation.DockDescription}:" />--%>
                </td>
                <td class="td02">
                    <sc1:CodeMstrLabel ID="lblApprovalStatus" runat="server" Code="ApprovalStatus" Value='<%# Bind("ApprovalStatus") %>' />
                    <%-- <asp:Literal ID="lbDockDescription" runat="server" Text='<%# Bind("DockDescription") %>' />--%>
                </td>
            </tr>
            <tr>
                <td class="td01">
                    <asp:Literal ID="lbDeliverType" runat="server" Text="${InProcessLocation.DeliverType}:" />
                </td>
                <td class="td02">
                    <sc1:CodeMstrDropDownList ID="ddlDeliverType" runat="server" Code="DeliverType" />
                </td>
                <td class="td01">
                    <asp:Literal ID="lblTransportCompany" runat="server" Text="${InProcessLocation.TransportCompany}:" />
                </td>
                <td class="td02">
                    <asp:TextBox ID="tbTransportCompany" runat="server" Text='<%# Bind("TransportCompany") %>' />
                </td>
            </tr>
            <tr>
                <td class="td01">
                ...
                <td class="td02">
                    <asp:TextBox ID="tbExpressNo" runat="server" Text='<%# Bind("ExpressNo") %>' />
                </td>
                <td class="td01">
                    <asp:Literal ID="lblDeliverDate" runat="server" Text="${InProcessLocation.DeliverDate}:" />
                </td>
                <td class="td02">
                    <asp:TextBox ID="tbDeliverDate" runat="server" Text='<%# Bind("DeliverDate") %>' onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" />
                </td>
            </tr>
            <%--
            <tr>
                <td class="td01">
                    <asp:Literal ID="lblCurrentOperation" runat="server" Text="${InProcessLocation.CurrentOperation}:" />
                </td>
                <td class="td02">
                    <uc3:textbox ID="tbCurrentOperation" runat="server" Visible="false" Width="250" DescField="Activity"
                        ValueField="Operation" ServicePath="InProcessLocationTrackMgr.service" ServiceMethod="GetInProcessLocationTrack"
                        Text='<%# Bind("CurrentOperation") %>' />
                    <sc1:ReadonlyTextBox ID="rtbCurrentOperation" runat="server" CodeField="CurrentOperation"
                        Visible="false" />
                </td>
                <td class="td01">
                    <asp:Literal ID="lblCurrentActivity" runat="server" Text="${InProcessLocation.CurrentActivity}:" />
                </td>
                <td class="td02">
                    <asp:TextBox ID="tbCurrentActivity" runat="server" Text='<%# Bind("CurrentActivity") %>'
                        onfocus="this.blur();" />
                </td>
            </tr>
            <tr>
