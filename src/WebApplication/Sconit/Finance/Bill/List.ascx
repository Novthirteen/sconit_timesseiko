        ...
        <cc1:GridView ID="GV_List" runat="server" AutoGenerateColumns="False" DataKeyNames="BillNo"
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="${MasterData.Bill.ExternalBillNo}" SortExpression="ExternalBillNo">
                    <ItemTemplate>
                        <%# DataBinder.Eval(Container.DataItem, "ExternalBillNo", "{0:0.########}")%>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:BoundField DataField="FixtureDate" DataFormatString="{0:yyyy-MM-dd}" HeaderText="${MasterData.Bill.PayableDate}"
                    SortExpression="FixtureDate" />
                
                <asp:TemplateField HeaderText="${MasterData.Bill.TotalDetailAmount}" SortExpression="AmountAfterDiscount">
                    <ItemTemplate>
                        <asp:Label ID="lblAmountAfterDiscount" runat="server" Text='<%# Bind("AmountAfterDiscount","{0:0.00}") %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="${MasterData.Bill.BackwashAmount}" SortExpression="BackwashAmount">
                    <ItemTemplate>
                ...
                <asp:TemplateField HeaderText="${Common.GridView.Action}">
                    <ItemTemplate>
                        <cc1:LinkButton ID="lbtnView" runat="server" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "BillNo") %>'
                            Text="${Common.Button.View}" OnClick="lbtnView_Click" FunctionId="ViewBill" Visible="false">
                        </cc1:LinkButton>
