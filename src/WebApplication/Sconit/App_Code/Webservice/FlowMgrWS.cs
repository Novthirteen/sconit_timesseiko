    ...
    public FlowDetailProxy GenerateFlowDetailProxy(string flowCode, string itemCode, string moduleType, string partyFromCode, string partyToCode, bool changeRef, DateTime startTime)
    {
                ...
                if (fd.Item.Code == itemCode)
                {
                        ...
                        || moduleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_DISTRIBUTION)
                    {
                        if (fd.DefaultPriceList != null)
                        {
                            ...
                            PriceListDetail priceListDetail = ThePriceListDetailMgr.GetLastestPriceListDetail(fd.DefaultPriceList, fd.Item, startTime, fd.Flow.Currency, fd.Uom);
                            if (priceListDetail != null)
                            {
                                flowDetailProxy.UnitPrice = priceListDetail.UnitPrice;
                                flowDetailProxy.PriceListDetailId = priceListDetail.Id;
                            }
                        }

                        if (moduleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_PROCUREMENT)
                        {
                            flowDetailProxy.LocationCode = fd.DefaultLocationTo.Code;
                        }
                        else if (moduleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_DISTRIBUTION)
                        {
                            flowDetailProxy.LocationCode = fd.DefaultLocationFrom.Code;
                        }
                    }
        ...
        if (item != null)
        {
                ...
                || moduleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_DISTRIBUTION)
            {
                ...
                if (moduleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_PROCUREMENT)
                {
                    flowDetailProxy.LocationCode = flow.LocationTo.Code;
                }
                else if (moduleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_DISTRIBUTION)
                {
                    flowDetailProxy.LocationCode = flow.LocationFrom.Code;
                }
            }
