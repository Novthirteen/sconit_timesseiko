...
public partial class Order_OrderDetail_List : ModuleBase
{
    ...
    private decimal TaxRate   //记录税率
    {
        get
        {
            return (decimal)ViewState["TaxRate"];
        }
        set
        {
    ...
    public bool IsShowDiscount   //显示折扣
    {
        get
        {
            return (bool)ViewState["IsShowDiscount"];
        }
        set
        {
            ViewState["IsShowDiscount"] = value;
        }
    }
    ...
    //保存
    public void SaveCallBack(bool isRecalculatePrice)
    {
            ...
            if (SaveEvent != null)
            {
                ...
                if (orderHead != null)
                {
                    ...
                    if (this.ModuleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_PROCUREMENT
                        || this.ModuleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_DISTRIBUTION)
                    {
                        orderHead.Discount = tbOrderDiscount.Text.Trim() == string.Empty ? 0 : decimal.Parse(tbOrderDiscount.Text.Trim());
                    }

                    if (orderHead.OrderDetails != null)
                    {
                        string taxCodeT = string.Empty;
                        if (isRecalculatePrice)
                        {
                            taxCodeT = TheEntityPreferenceMgr.LoadEntityPreference(BusinessConstants.ENTITY_PREFERENCE_CODE_TAX_RATE).Value;
                        ...
                        for (int i = 0; i < orderHead.OrderDetails.Count; i++)
                        {
                            ...
                            if (isRecalculatePrice)
                            {
                                if (orderHead != null)
                                {
                                    ...
                                    {
                                        Flow flow = this.TheFlowMgr.LoadFlow(orderHead.Flow);
                                        if (flow != null)
                                        {
                                    ...
                                    {
                                        PriceListDetail priceListDetail = ThePriceListDetailMgr.GetLastestPriceListDetail(orderDetail.DefaultPriceList, orderDetail.Item, orderHead.StartTime, orderHead.Currency, orderDetail.Uom);
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            this.ucView.ModuleSubType = this.ModuleSubType;
            //默认取企业选项里控制价格
            ...
            //默认取企业选项里税率比例
            //this.TaxRate = decimal.Parse(TheEntityPreferenceMgr.LoadEntityPreference(BusinessConstants.ENTITY_PREFERENCE_CODE_TAX_RATE).Value);

            //this.hfTax.Value = this.TaxRate.ToString();

            //默认取企业选项里是否显示折扣
            this.IsShowDiscount = bool.Parse(TheEntityPreferenceMgr.LoadEntityPreference(BusinessConstants.ENTITY_PREFERENCE_CODE_IS_SHOW_DISCOUNT).Value);
    ...
    protected void GV_List_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            #region 价格，折扣
            if (this.ModuleSubType == BusinessConstants.CODE_MASTER_ORDER_SUB_TYPE_VALUE_NML && this.IsShowPrice)
            {
                if (true || orderDetail.IsBlankDetail)
                {
                    if (this.ModuleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_PROCUREMENT
                        || this.ModuleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_DISTRIBUTION)
                    {
                ...
                else
                {
                         ...
                         || this.ModuleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_DISTRIBUTION)
                    {
                        ...
                        {
                            tbPrice.Text = (orderDetail.UnitPrice * orderDetail.OrderedQty).Value.ToString("F2");
                        }
                    }

                }

                this.trdiscount.Visible = this.IsShowDiscount;
                this.trOrderPrice.Visible = this.IsShowDiscount;

                tbDiscount.Attributes.Add("onchange", "discountChanged(this," + orderDetail.IsBlankDetail.ToString().ToLower() + ");");
            ...
            if (this.ModuleSubType != null && this.ModuleSubType != string.Empty)
            {
                ...
                if (this.ModuleSubType == BusinessConstants.CODE_MASTER_ORDER_SUB_TYPE_VALUE_NML || this.ModuleSubType == BusinessConstants.CODE_MASTER_ORDER_SUB_TYPE_VALUE_RWO)
                {
                    if (this.NewOrEdit == "New" || orderDetail.IsBlankDetail)
                    {
                        revOrderQty.ValidationExpression = "^(0|([1-9]\\d*))(\\.\\d+)?$";
                    }
                    else
                    {
                        revOrderQty.ValidationExpression = "^(([1-9]\\d*))(\\.\\d+)?$";
                    }
    ...
    protected void lbtnAdd_Click(object sender, EventArgs e)
    {
        ...
        if (rfvItemCode.IsValid && rfvUom.IsValid)
        {
            ...
            if (orderDetailList != null)
            {
                ...
                TextBox tbNewUnitPrice = (TextBox)newOrderDetailRow.FindControl("tbUnitPrice");
                ...
                HiddenField hfLocationCode = (HiddenField)newOrderDetailRow.FindControl("hfLocationCode");

                int seqInterval = int.Parse(TheEntityPreferenceMgr.LoadEntityPreference(BusinessConstants.ENTITY_PREFERENCE_CODE_SEQ_INTERVAL).Value);
                decimal orderQty = tbNewOrderQty.Text.Trim() == string.Empty ? 0 : decimal.Parse(tbNewOrderQty.Text.Trim());
                decimal unitPrice = tbNewUnitPrice.Text.Trim() == string.Empty ? 0 : decimal.Parse(tbNewUnitPrice.Text.Trim());
                ...
                for (int i = 0; i < newItemList.Count; i++)
                {
                    ...
                    if (newOrderDetail.Item.Code != newItem.Code)
                    {
                        ...
                        if (this.IsShowPrice)
                        {
                                ...
                                || this.ModuleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_DISTRIBUTION)
                            {
                                ...
                                {
                                    newOrderDetail.UnitPrice = this.ThePriceListDetailMgr.GetLastestPriceListDetail(newOrderDetail.PriceList, item, DateTime.Now, newOrderDetail.OrderHead.Currency, item.Uom).UnitPrice;
                                }
                            }
                        }
                        #endregion
                    ...
                    else
                    {
                        ...
                        if (this.IsShowPrice)
                        {
                                ...
                                || this.ModuleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_DISTRIBUTION)
                            {
                                ...
                                if (hfPriceListDetailId.Value != string.Empty)
                                {
                                    ...
                                    if (priceListDetail != null)
                                    {
                                        ...
                                        newOrderDetail.TaxCode = taxCode; //priceListDetail.TaxCode;
                    ...
                    if (this.ModuleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_PROCUREMENT)
                    {
                        if (hfLocationCode.Value != string.Empty)
                        {
                            newOrderDetail.LocationTo = TheLocationMgr.LoadLocation(hfLocationCode.Value.Trim());
                        }
                    ...
                    else if (this.ModuleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_DISTRIBUTION)
                    {
                        if (hfLocationCode.Value != string.Empty)
                        {
                            newOrderDetail.LocationFrom = TheLocationMgr.LoadLocation(hfLocationCode.Value.Trim());
                        }
                    }
                    #endregion
    ...
    public void CleanPrice()
    {
        ...
        tbOrderTax.Text = string.Empty;
        tbOrderIncludeTaxPrice.Text = string.Empty;
    }
    ...
    public void InitPageParameter()
    {
        ...
        {
            UpdateSavedOrderHead();
            InitPageParameter(orderHead);

            CalculateOrderPrice();
            this.tbOrderDiscount.Text = "0";
            this.tbOrderDiscountRate.Text = "0";
            this.tbOrderPrice.Text = this.tbOrderDetailPrice.Text;
    ...
    private void CalculateOrderPrice()
    {
        ...
        for (int i = 0; i < GetOrderDetailList().Count; i++)
        {
            OrderDetail orderDetail = GetOrderDetailList()[i];
            decimal detailPrice = 0;

            if (this.ModuleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_PROCUREMENT)
            {
                detailPrice = orderDetail.UnitPrice.Value * orderDetail.OrderedQty;
            }
            else if (this.ModuleType == BusinessConstants.CODE_MASTER_ORDER_TYPE_VALUE_DISTRIBUTION)
            {
                detailPrice = orderDetail.UnitPrice.Value * orderDetail.OrderedQty;
            ...
            TextBox tbPrice = (TextBox)this.GV_List.Rows[i].FindControl("tbPrice");
            tbPrice.Text = detailPrice.ToString("F2");
            tbPrice.Attributes["oldValue"] = tbPrice.Text;
            totalPrice += detailPrice;
        }
        this.tbOrderDetailPrice.Text = totalPrice.ToString("F2");
    }
}
