        ...
        <cc1:GridView ID="GV_List" runat="server" AutoGenerateColumns="False" DataKeyNames="Id"
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="${MasterData.Order.ActingBill.UnitPrice}" SortExpression="UnitPrice"
                    Visible="false">
                    <ItemTemplate>
                        <asp:Label ID="lbUnitPrice" runat="server" Text='<%# Bind("UnitPrice","{0:0.00}") %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="${MasterData.Order.ActingBill.Discount}" SortExpression="Discount"
                    Visible="false">
                    <ItemTemplate>
                        <asp:Label ID="lbDiscount" runat="server" Text='<%# Bind("Discount","{0:0.00}") %>' />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="${MasterData.Order.ActingBill.Amount}" SortExpression="TotalAmount"
                    Visible="false">
                    <ItemTemplate>
                        <asp:Label ID="lbTotalAmount" runat="server" Text='<%# Bind("Amount","{0:0.00}") %>' />
                    </ItemTemplate>
                </asp:TemplateField>
