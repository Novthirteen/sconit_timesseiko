...
<div id="divPassword" visible="false" runat="server">
    <fieldset>
        ...
        <table width="100%">
            ...
            <tr>
                <td class="td01">
                    ${Security.User.EnableUsbKey}:
                </td>
                <td class="td02">
                    <table>
                        <tr>
                            <td>
                                <asp:RadioButtonList ID="rblEnableUsbKey" RepeatDirection="Horizontal" runat="server">
                                    <asp:ListItem Text="否" Value="false" />
                                    <asp:ListItem Text="是" Value="true" />
                                </asp:RadioButtonList>
                            </td>
                            <td>
                                <asp:Button ID="btnSave" runat="server" OnClick="btnUpdatePassword_Click" Text="${Common.Button.Save}" />
                            </td>
                        </tr>
                    </table>
                </td>
                ...
                <td class="td02">
                    <div class="buttons">
                        <asp:Button ID="btnUpdatePassword" runat="server" OnClick="btnUpdatePassword_Click"
                            Text="${Security.User.UpdatePassword}" CssClass="apply" ValidationGroup="vgPassword" />
                        <asp:Button ID="btnUpdateUsbKey" runat="server" OnClick="btnUpdatePassword_Click"
                            OnClientClick="var confirm=GetUsbKey();return confirm;" Text="${Security.User.UpdateUsbKey}" />
                        <asp:Button ID="btnCancel" runat="server" OnClick="btnCancel_Click" Text="${Common.Button.Cancel}"
                            CssClass="cancel" />
                        <input id="UsbKeyCount" class="UsbKeyCount" runat="server" type="hidden" />
                        <input id="UsbKey" class="UsbKey" runat="server" type="hidden" />
                    </div>
                </td>
            </tr>
        </table>
