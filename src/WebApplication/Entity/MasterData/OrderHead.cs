...
namespace com.Sconit.Entity.MasterData
{
    ...
    public class OrderHead : OrderHeadBase
    {
        ...
        public decimal OrderTax
        {
            get
            {
                if ((!this.IsIncludeTax.HasValue || !this.IsIncludeTax.Value) && this.TaxCode != null)
                {
                    return this.OrderAmountAfterDiscount * decimal.Parse(this.TaxCode) / 100;
                }
                else
                {
                    return this.OrderAmountAfterDiscount / (decimal.Parse(this.TaxCode) / 100 + 1) * (decimal.Parse(this.TaxCode) / 100);
                }
            }
        }

        public decimal OrderExcludeTaxPrice
        {
            get
            {
                if (this.TaxCode != null)
                {
                    if (!this.IsIncludeTax.HasValue || !this.IsIncludeTax.Value)
                    {
                        return this.OrderAmountAfterDiscount;
                    }
                    else
                    {
                        return this.OrderAmountAfterDiscount / (decimal.Parse(this.TaxCode) / 100 + 1);
                    }
                }
                else
                {
        ...
        public decimal OrderIncludeTaxPrice
        {
            get
            {
                if (this.TaxCode != null)
